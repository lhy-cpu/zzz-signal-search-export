    ...
    try {
      ...
      data.result.forEach((value, key) => {
        value.forEach(item => {
          ...
          if(idJson["zh-cn"][item.item_id]==undefined){
            idJson["zh-cn"][item.item_id]=
            {
              "name": item.name,
...
const readLog = async () => {
  ...
  try {
    ...
    const promises = logPaths.map(async logpath => {
      ...
      const gamePathMch = logText.match(/\w:\/.*?\/ZenlessZoneZero_Data\//i)
      if (gamePathMch) {
        const [cacheText, cacheFile] = await getCacheText(gamePathMch[0])
        const urlMch = cacheText.match(/https[^?]+?\?[^?]+?&auth_appid=webview_gacha&.+?plat_type=pc&authkey=.+?&game_biz=nap_cn/g)
        if (urlMch) {
...
const getGachaLog = async ({ key, page, name, retryCount, url, endId }) => {
  const text = i18n.log
  try {
    console.log(`${url}&page=${page}&size=${20}&gacha_type=${key}&real_gacha_type=${key[0]}${endId ? '&end_id=' + endId : ''}`)
    const res = await request(`${url}&page=${page}&size=${20}&gacha_type=${key}&real_gacha_type=${key[0]}${endId ? '&end_id=' + endId : ''}`)
    if (res?.data?.list) {
      return res?.data
    }
...
const tryGetUid = async (queryString) => {
  ...
  try {
    for (let [key] of defaultTypeMap) {
      const res = await request(`${url}&gacha_type=${key}&real_gacha_type=${key[0]}&page=1&size=6`)
...
const getQuerystring = (url) => {
  ...
  if (host.includes('webstatic-sea') || host.includes('nap-api-os') || host.includes('api-os-takumi') || host.includes('hoyoverse.com')) {
...
const tryRequest = async (url, retry = false) => {
  ...
  const gachaTypeUrl = `${apiDomain}/common/gacha_record/api/getGachaLog?${queryString}&page=1&size=5&gacha_type=1001&real_gacha_type=1&end_id=0`
  try {
